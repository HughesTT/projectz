...
const discountAmount = computed(() => {
  if (!appliedCoupon.value || !appliedCoupon.value.discountAmount) return 0
  return Number(appliedCoupon.value.discountAmount) || 0
})

// 總計（如果套用優惠券，使用後端回傳的 final_total）
const finalTotal = computed(() => {
  const total = Number(cartTotal.value) || 0
  const shipping = total > 499 ? 0 : 100
  shippingFee.value = shipping

  // 如果有套用優惠券且後端有回傳 final_total，直接使用
  if (appliedCoupon.value && appliedCoupon.value.backendFinalTotal) {
    return Number(appliedCoupon.value.backendFinalTotal) || 0
  }
...
const applyCoupon = async () => {
  ...
  try {
    ...
    if (result.success) {
      const backendFinalTotal = result.data?.final_total // 後端直接計算的最終價格
      ...
      if (backendFinalTotal) {
        ...
        showToast(result.message || '折價券套用成功！', 'success')
      }
    } else {
...
const handleCheckout = async () => {
  ...
  try {
    ...
    // 步驟 1: 將購物車商品同步到後端購物車（如果還沒套用優惠券）
    // 如果已套用優惠券，購物車在驗證優惠券時已經同步過了，避免重複同步覆蓋優惠券
    if (!appliedCoupon.value) {
      for (const item of cart.value) {
        ...
        const cartResult = await cartRes.json()
        if (!cartResult.success) {
          throw new Error(`加入購物車失敗：${cartResult.message}`)
        }
      }
    } else {
      showToast('購物車已同步，正在處理訂單...', 'info')
    }

    // 步驟 2: 準備訂單資料（包含 user 資訊、備註和優惠券代碼）
    const orderData = {
      user: {
    ...
    if (appliedCoupon.value && appliedCoupon.value.code) {
      ...
      console.log('🎫 訂單包含優惠券:', appliedCoupon.value.code)
    }
    ...
    if (result.success) {
      console.log('✅ 後端訂單建立成功，訂單編號:', result.orderId)
      showToast('訂單建立成功！', 'success')

      // 2️⃣ 同時儲存到 localStorage 供前端查詢
      const localOrder = {
        ...
        paymentInfo: {
          cardLastFour: cardInfo.value.number.slice(-4),
          cardHolder: cardInfo.value.name
        },
